...
## Tek cümlelik ürün
Kullanıcı, masaüstü oyunlarının kendi skor kağıdının **fotoğrafını** çekip dijital "yeni kopya" gibi kullanır: fotoğrafın üzerine **parmağıyla/kalemle skoru yazar** ve her oynanışı **geçmişte** saklar. Fiziksel kağıtları bittiği için bu uygulamayı kullanırlar.

**Karar (2026-09-09) — satır işaretleme yok.** Kağıt kurulumu yalnızca foto + isimdir; kullanıcı oyun sırasında kağıdın **tamamına** yazar, izler bir satıra bağlanmaz (`strokes.row_id` hep boş). `sheet_rows` tablosu şemada kalır ama arayüzde kullanılmaz; tanıma fazı (Faz 5) gelirse satırlar otomatik tespit ya da sonradan tanımlama ile eklenir, kullanıcıya baştan işaretletilmez.
...
2. **Koordinatlar piksel değil orandır.** Kalem izleri (strokes) ve ileride eklenirse satır konumları her zaman `0..1` aralığında oran olarak saklanır; ekran boyutundan/zoomdan bağımsız çalışır.
